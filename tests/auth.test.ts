...
test("auth checker rejects request with no Authorization header when token is configured", () => {
  const auth = createAuthChecker({ MEMORY_V2_API_TOKEN: "secret-key-123" });
...
test("auth checker allows all requests when MEMORY_V2_API_TOKEN is empty (dev mode)", () => {
  const auth = createAuthChecker({ MEMORY_V2_API_TOKEN: "" });
