...
test("orchestrator forget-memory synchronizes tombstone projection after commit", async () => {
  const projectionResults: Array<{ memoryId: string; lifecycleStatus: string }> = [];
  const harness = await createTestHarness({
    projectionSyncService: {
      async syncWriteResult(result: { memoryId: string; lifecycleStatus: string }) {
  ...
  try {
    const write = await request(harness.baseUrl, "/api/memory/xx/write", {
      method: "POST",
      body: {
        requestId: "http-forget-projection-seed",
        actorId: "http-test",
        scopeType: "project",
        scopeId: "http-forget-projection",
        content: "HTTP orchestrator forget should delete the Qdrant projection.",
        lifecycleStatus: "approved",
        reviewState: "not_required",
      },
    });
    assert.equal(write.status, 201);
    const memoryId = String((write.body as Record<string, unknown>).memoryId ?? "");
    assert.ok(memoryId);
    projectionResults.length = 0;

    const forget = await request(harness.baseUrl, "/api/memory/xx/orchestrator/forget-memory", {
      method: "POST",
      body: {
        requestId: "http-forget-projection-tombstone",
        actorId: "http-test",
        memoryId,
        mode: "tombstone",
      },
    ...
    assert.deepEqual(projectionResults, [{ memoryId, lifecycleStatus: "tombstone" }]);
  } finally {
    await harness.close();
  }
});
