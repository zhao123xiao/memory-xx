...
test("public repository exposes runtime observability smoke for retention dry-runs", async () => {
  const files = [
    "app/observability/domain-metrics.ts",
    "app/observability/mcp-tool-invocations.ts",
    "app/ops/runtime-artifacts-cleanup.ts",
    ...
    "tests/runtime-observability-smoke.test.ts",
  ];
  const missing: string[] = [];
  const stale: string[] = [];
  for (const file of files) {
  ...
  assert.deepEqual(missing, []);
  assert.deepEqual(stale, []);
  assert.equal(packageJson.scripts["smoke:runtime-observability"], "node --import tsx scripts/runtime-observability-smoke.ts");
...
test("public repository exposes trusted agent smoke for strict-scope provisioning audits", async () => {
  const files = [
    "app/server/permissions.ts",
    "app/orchestrator/scope-plan.ts",
    "app/mcp/tool-registry.ts",
    "scripts/memory-trusted-agent.ts",
    "scripts/memory-agent.ts",
    "scripts/trusted-agent-smoke.ts",
    "tests/trusted-agent-smoke.test.ts",
  ];
  const missing: string[] = [];
  const stale: string[] = [];
  for (const file of files) {
  ...
  const operationsZh = await readFile("docs/operations.zh-CN.md", "utf8");
  const moduleCatalog = await readFile("docs/module-catalog.md", "utf8");
  const agentDocs = await readFile("docs/agent-integration.zh-CN.md", "utf8");

  assert.deepEqual(missing, []);
  assert.deepEqual(stale, []);
  assert.equal(packageJson.scripts["smoke:trusted-agent"], "node --import tsx scripts/trusted-agent-smoke.ts");
  ...
  assert.match(agentDocs, /TMPDIR=\/tmp npm run smoke:trusted-agent/u);
});

test("public repository exposes embedding ops smoke for manifest status and calibration", async () => {
  const files = [
    "app/embedding/generation-manifest.ts",
    "app/embedding/manifest-refresh.ts",
    "app/server/embedding-provider.ts",
    "app/recall/query-embedding-resilience.ts",
    "scripts/embedding-manifest.ts",
    "scripts/embedding-calibration.ts",
    "scripts/embedding-ops-smoke.ts",
    "tests/embedding-ops-smoke.test.ts",
  ];
  const missing: string[] = [];
  const stale: string[] = [];
  for (const file of files) {
  ...
  assert.deepEqual(missing, []);
  assert.deepEqual(stale, []);
  assert.equal(packageJson.scripts["smoke:embedding-ops"], "node --import tsx scripts/embedding-ops-smoke.ts");
  assert.match(packageJson.scripts["verify:open-source"], /tests\/embedding-ops-smoke\.test\.ts/u);
...
test("public repository exposes self-improvement ops smoke for report-only proposals", async () => {
  const files = [
    ...
    "tests/self-improvement-ops-smoke.test.ts",
  ];
  const missing: string[] = [];
  const stale: string[] = [];
  for (const file of files) {
  ...
  assert.deepEqual(missing, []);
  assert.deepEqual(stale, []);
  assert.equal(packageJson.scripts["smoke:self-improvement-ops"], "node --import tsx scripts/self-improvement-ops-smoke.ts");
  assert.match(packageJson.scripts["verify:open-source"], /tests\/self-improvement-ops-smoke\.test\.ts/u);
  assert.match(capabilities, /name: "self_improvement_ops"[\s\S]*scripts\/self-improvement-ops-smoke\.ts/u);
  assert.match(smoke, /memory-self-improvement\.ts/u);
  assert.match(smoke, /memory-graphiti-shadow-export\.ts/u);
  assert.match(smoke, /memory-sweep-test-pollution\.ts/u);
  assert.match(smoke, /--dry-run/u);
