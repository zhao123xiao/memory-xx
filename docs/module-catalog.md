...
| `memory_knowledge_graph` | `memory:graph-report`, `memory:graph-health`, `memory:graph-relation-repair-apply`, `smoke:knowledge-graph` |
| `code_graph` | `memory:code-graph` |
| `temporal_decay` | `memory:decay`, `memory:temporal-sweep`, `memory:temporal-policy`, `memory:stale-fact-report`, `smoke:temporal-ops` |
| `temporal_consolidation` | `memory:consolidate`, `memory:consolidation-candidates`, `smoke:temporal-ops` |
...
| `recall_quality` | `memory:quality`, `memory:intelligence-quality`, `memory:reranker-policy-benchmark`, `memory:p1-evidence`, `memory:recall-repair`, `memory:adaptive-retrieval-apply`, `memory:adaptive-retrieval-calibration`, `memory:extraction-recall-eval`, `memory:recall-quality-feedback`, `memory:trace-feedback`, `smoke:recall-quality` |
...
| `governance_operations` | `memory:governance-audit`, `memory:governance-cleanup`, `memory:governance-freeze`, `memory:governance-revert`, `memory:policy-backfill`, `memory:pending-canary-report`, `memory:pending-governance`, `memory:pending`, `memory:governance`, `memory:governance-dry-run-jobs`, `memory:governance-stuck-runs`, `memory:memory-type-backfill`, `memory:event-lifecycle`, `memory:archive-events`, `memory:human-review-apply`, `smoke:governance-ops` |
...
| `release_governance_gates` | `memory:p0-gate`, `memory:p1-gate`, `memory:cutover-gate`, `memory:landing-scan`, `memory:canary-7d-report`, `memory:freeze-m0`, `memory:capacity-audit`, `memory:capacity-smoke`, `memory:consistency-scan`, `memory:dlq-recovery`, `memory:quality-metadata-backfill`, `shadow:recall`, `shadow:projection`, `memory:parity-audit`, `verify:open-source`, `verify:open-source-full-stack`, `smoke:full-ops` |
| `self_improvement_ops` | `memory:self-improvement`, `memory:evolve`, `memory:observation-reflection`, `memory:procedural-promotion-candidates`, `memory:graphiti-shadow-export`, `memory:sweep-test-pollution`, `smoke:self-improvement-ops` |
