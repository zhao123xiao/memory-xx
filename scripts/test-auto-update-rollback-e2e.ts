...
async function seedPair(client: import("pg").PoolClient, schema: string, runId: string): Promise<{ scopeId: string; oldId: string; candidateId: string }> {
  ...
  await client.query(
    `INSERT INTO ${schema}.ingest_requests (request_id, command_type, payload_hash, payload_json, actor_id, status, completed_at, result_json)
     VALUES ($1, 'memory.create', $2, '{}'::jsonb, 'test:auto-update-rollback-e2e', 'completed', now(), '{}'::jsonb),
            ($3, 'memory.create', $4, '{}'::jsonb, 'test:auto-update-rollback-e2e', 'completed', now(), '{}'::jsonb)`,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const dryRun = await runCli(["memory:auto-update", "--", "rollback", `--decision-id=${String(apply.decision_id)}`, "--dry-run"]);
    const rollback = await runCli(["memory:auto-update", "--", "rollback", `--decision-id=${String(apply.decision_id)}`, "--apply"]);
    ...
    const governance = await client.query(`SELECT status, reverted_at, evidence FROM ${schema}.memory_governance_actions WHERE id = $1`, [apply.decision_id]);
    const ok = dryRun.ok === true &&
      rollback.ok === true &&
      old?.lifecycle_status === "approved" &&
      old?.is_current === true &&
      newer?.lifecycle_status === "tombstone" &&
      newer?.is_current === false &&
      governance.rows[0]?.status === "reverted" &&
      Boolean(governance.rows[0]?.reverted_at);
    const report = { ok, run_id: runId, scope: `project:${seeded.scopeId}`, decision_id: apply.decision_id, old_memory_id: seeded.oldId, new_memory_id: seeded.candidateId, apply, dry_run: dryRun, rollback, rows: rows.rows, governance: governance.rows[0] ?? null };
    const reportDir = join(process.cwd(), "reports", "auto-update-rollback-e2e");
    await mkdir(reportDir, { recursive: true });
    const reportPath = join(reportDir, `auto-update-rollback-e2e-${runId}.json`);
