...
import { loadMemoryXXPostgresConfig, PostgresWriteDatabase } from "../app/db";

function loadDefaultMetricsEnv(): void {
  if (process.env.MEMORY_XX_P1_GATE_METRICS_JSON?.trim()) return;
  const explicit = process.env.MEMORY_XX_P1_GATE_METRICS_FILE?.trim();
  ...
  process.env.MEMORY_XX_P1_GATE_METRICS_JSON = readFileSync(file, "utf8");
}

function loadDefaultPolicyTrainingEnv(): void {
  if (process.env.MEMORY_XX_POLICY_TRAINING_SUMMARY_JSON?.trim()) return;
  const explicit = process.env.MEMORY_XX_POLICY_TRAINING_SUMMARY_FILE?.trim();
  if (explicit && existsSync(explicit)) {
    process.env.MEMORY_XX_POLICY_TRAINING_SUMMARY_JSON = readFileSync(explicit, "utf8");
    return;
  }
  ...
  if (latest) process.env.MEMORY_XX_POLICY_TRAINING_SUMMARY_JSON = readFileSync(latest, "utf8");
}

async function main(): Promise<void> {
  loadDefaultMetricsEnv();
  loadDefaultPolicyTrainingEnv();
  if (!process.env.MEMORY_XX_DATABASE_URL?.trim()) {
