...
export function defaultControlPanelRuntimeSettings(): ControlPanelRuntimeSettings {
  return {
    version: 1,
    graph: {
      default_scope_type: process.env.MEMORY_V2_CONTROL_PANEL_GRAPH_SCOPE_TYPE?.trim() || "workspace",
      default_scope_id: process.env.MEMORY_V2_CONTROL_PANEL_GRAPH_SCOPE_ID?.trim() || "current-instance",
      default_limit: 80,
    },
    panel: {
...
export function buildRuntimeRegistry(env: NodeJS.ProcessEnv = process.env): RuntimeRegistryItem[] {
  ...
  return [
    ...
    registryItem({ key: "cache.redis.ttl.search_seconds", label: "搜索缓存过期时间（Redis search TTL）", description: "召回搜索结果在 Redis（缓存服务）中保留的秒数。", category: "cache", type: "number", unit: "seconds", default_value: 300, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 5, max: 86_400, step: 5, env_key: "MEMORY_V2_REDIS_TTL_SEARCH_SECONDS" }, env),
    registryItem({ key: "cache.redis.ttl.session_seconds", label: "会话缓存过期时间（Redis session TTL）", description: "会话召回缓存保留秒数。", category: "cache", type: "number", unit: "seconds", default_value: 1800, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 30, max: 86_400, step: 30, env_key: "MEMORY_V2_REDIS_TTL_SESSION_SECONDS" }, env),
    registryItem({ key: "cache.redis.ttl.recent_seconds", label: "最近记忆缓存过期时间（Redis recent TTL）", description: "最近记忆缓存保留秒数。", category: "cache", type: "number", unit: "seconds", default_value: 900, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 30, max: 86_400, step: 30, env_key: "MEMORY_V2_REDIS_TTL_RECENT_SECONDS" }, env),
    registryItem({ key: "cache.redis.ttl.startup_context_seconds", label: "启动上下文缓存过期时间（Redis startup-context TTL）", description: "项目启动上下文召回缓存保留秒数。", category: "cache", type: "number", unit: "seconds", default_value: 600, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 30, max: 86_400, step: 30, env_key: "MEMORY_V2_REDIS_TTL_STARTUP_CONTEXT_SECONDS" }, env),
    registryItem({ key: "cache.redis.empty_recall_ttl_seconds", label: "空召回缓存过期时间（Empty recall TTL）", description: "空结果召回缓存保留秒数，过长可能掩盖新写入。", category: "cache", type: "number", unit: "seconds", default_value: 15, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1, max: 3600, step: 1, env_key: "MEMORY_V2_EMPTY_RECALL_CACHE_TTL_SECONDS" }, env),
    registryItem({ key: "cache.redis.connect_timeout_ms", label: "缓存服务连接超时（Redis connect timeout）", description: "Redis 客户端连接超时时间。", category: "cache", type: "number", unit: "ms", default_value: 2000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 100, max: 30_000, step: 100, env_key: "MEMORY_V2_REDIS_CONNECT_TIMEOUT_MS" }, env),
    registryItem({ key: "cache.reranker.ttl_ms", label: "重排序缓存过期时间（Reranker cache TTL）", description: "模型重排序分数映射的本地缓存过期时间，新召回请求热生效。", category: "recall", type: "number", unit: "ms", default_value: 60_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 0, max: 3_600_000, step: 1000, env_key: "MEMORY_V2_RERANKER_CACHE_TTL_MS" }, env),
    ...
    registryItem({ key: "write.rate_limit.max_requests", label: "写入/API 请求速率上限（rate limit max）", description: "单窗口允许请求数；运行时热更新后新请求立即使用。", category: "write", type: "number", unit: "requests", default_value: 60, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1, max: 10_000, step: 1, env_key: "MEMORY_V2_RATE_LIMIT_MAX" }, env),
    registryItem({ key: "write.rate_limit.window_ms", label: "写入/API 请求速率窗口（rate limit window）", description: "请求速率限制的统计窗口长度。", category: "write", type: "number", unit: "ms", default_value: 60_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1000, max: 3_600_000, step: 1000, env_key: "MEMORY_V2_RATE_LIMIT_WINDOW_MS" }, env),
    registryItem({ key: "write.ticket.ttl_seconds", label: "写入票据过期时间（Write ticket TTL）", description: "智能写入票据默认有效期，新 fast_ack（快速确认）请求热生效。", category: "write", type: "number", unit: "seconds", default_value: 120, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 10, max: 3600, step: 10, env_key: "MEMORY_V2_WRITE_TICKET_TTL_SECONDS" }, env),
    registryItem({ key: "write.semantic_lock.ttl_ms", label: "语义写入锁过期时间（Semantic lock TTL）", description: "同 scope（作用域）语义写入锁的最大持有时间，新 smart-write（智能写入）热生效。", category: "write", type: "number", unit: "ms", default_value: 30_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1000, max: 300_000, step: 1000, env_key: "MEMORY_V2_SEMANTIC_LOCK_TTL_MS" }, env),
    registryItem({ key: "write.semantic_lock.wait_timeout_ms", label: "语义写入锁等待超时（Semantic lock wait timeout）", description: "发现相似写入正在进行时最多等待多久，新智能写入热生效。", category: "write", type: "number", unit: "ms", default_value: 5_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 100, max: 60_000, step: 100, env_key: "MEMORY_V2_SEMANTIC_LOCK_WAIT_TIMEOUT_MS" }, env),
    registryItem({ key: "write.smart_write.qdrant_preflight_timeout_ms", label: "智能写入向量预检超时（Smart-write Qdrant preflight timeout）", description: "智能写入前查询 Qdrant（向量库）相似记忆的超时时间，新请求热生效。", category: "write", type: "number", unit: "ms", default_value: 200, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 20, max: 10_000, step: 10, env_key: "MEMORY_V2_SMART_WRITE_QDRANT_PREFLIGHT_TIMEOUT_MS" }, env),
    ...
    registryItem({ key: "recall.graph_health_ttl_ms", label: "图谱健康报告有效期（Graph health TTL）", description: "图谱健康 guard（保护检查）缓存有效期，新召回请求热生效。", category: "recall", type: "number", unit: "ms", default_value: 86_400_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1000, max: 604_800_000, step: 1000, env_key: "MEMORY_V2_GRAPH_HEALTH_TTL_MS" }, env),
    registryItem({ key: "recall.reranker.timeout_ms", label: "重排序模型超时（Reranker timeout）", description: "模型重排序 HTTP 调用超时时间，新召回请求热生效。", category: "recall", type: "number", unit: "ms", default_value: 1500, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 100, max: 60_000, step: 100, env_key: "MEMORY_V2_RERANKER_TIMEOUT_MS" }, env),
    registryItem({ key: "recall.fastpath.primary_timeout_ms", label: "快速召回主路径超时（Fastpath primary timeout）", description: "Go fastpath（快速召回路径）主召回路径超时，新召回请求热生效。", category: "recall", type: "number", unit: "ms", default_value: 10_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 100, max: 60_000, step: 100, env_key: "MEMORY_V2_FASTPATH_PRIMARY_TIMEOUT_MS" }, env),
    registryItem({ key: "recall.fastpath.shadow_timeout_ms", label: "快速召回影子路径超时（Fastpath shadow timeout）", description: "shadow recall（影子召回观测路径）超时时间。", category: "recall", type: "number", unit: "ms", default_value: 10_000, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-wrapper.service", safety: "guarded", min: 100, max: 60_000, step: 100, env_key: "MEMORY_V2_FASTPATH_SHADOW_TIMEOUT_MS" }, env),
    registryItem({ key: "worker.qdrant_projector.interval_ms", label: "向量投影轮询间隔（Qdrant projector interval）", description: "Qdrant（向量库）投影 worker（后台任务）空闲轮询间隔。", category: "worker", type: "number", unit: "ms", default_value: 5000, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-qdrant-projector-worker.service", safety: "guarded", min: 500, max: 300_000, step: 500, env_key: "MEMORY_V2_QDRANT_PROJECTOR_INTERVAL_MS" }, env),
    registryItem({ key: "worker.qdrant_projector.batch_size", label: "向量投影批量大小（Qdrant projector batch size）", description: "每轮处理 outbox（投影事件队列）事件数量。", category: "worker", type: "number", unit: "items", default_value: 50, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-qdrant-projector-worker.service", safety: "guarded", min: 1, max: 1000, step: 1, env_key: "MEMORY_V2_QDRANT_PROJECTOR_BATCH_SIZE" }, env),
    registryItem({ key: "worker.qdrant_projector.max_attempts", label: "投影最大重试次数（Qdrant projector max attempts）", description: "投影失败进入 dead-letter（死信队列）前的最大尝试次数。", category: "worker", type: "number", unit: "attempts", default_value: 5, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-qdrant-projector-worker.service", safety: "guarded", min: 1, max: 100, step: 1, env_key: "MEMORY_V2_QDRANT_PROJECTOR_MAX_ATTEMPTS" }, env),
    registryItem({ key: "worker.qdrant_projector.retry_delay_ms", label: "投影重试延迟（Qdrant projector retry delay）", description: "Qdrant 投影失败后的基础重试延迟。", category: "worker", type: "number", unit: "ms", default_value: 5000, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-qdrant-projector-worker.service", safety: "guarded", min: 100, max: 300_000, step: 100, env_key: "MEMORY_V2_QDRANT_PROJECTOR_RETRY_DELAY_MS" }, env),
    ...
    registryItem({ key: "worker.conversation.poll_interval_ms", label: "对话监听轮询间隔（Conversation monitor interval）", description: "对话监听器检查 JSONL/HTTP spool（写入缓冲）的间隔。", category: "worker", type: "number", unit: "ms", default_value: 1000, writable: true, hot_reloadable: false, requires_restart: true, service: "openclaw-conversation-monitor-worker.service", safety: "guarded", min: 100, max: 60_000, step: 100, env_key: "MEMORY_V2_CONVERSATION_POLL_INTERVAL_MS" }, env),
    registryItem({ key: "worker.write_ticket.interval_ms", label: "写入票据 worker 间隔（Write ticket worker interval）", description: "写入票据 worker 主循环 sleep（休眠）间隔。", category: "worker", type: "number", unit: "ms", default_value: 1000, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-write-ticket-worker.service", safety: "guarded", min: 100, max: 60_000, step: 100, env_key: "MEMORY_V2_WRITE_TICKET_WORKER_INTERVAL_MS" }, env),
    registryItem({ key: "worker.write_ticket.batch_size", label: "写入票据批量大小（Write ticket worker batch size）", description: "写入票据 worker 每轮处理票据数量。", category: "worker", type: "number", unit: "items", default_value: 10, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-write-ticket-worker.service", safety: "guarded", min: 1, max: 1000, step: 1, env_key: "MEMORY_V2_WRITE_TICKET_WORKER_BATCH_SIZE" }, env),
    registryItem({ key: "worker.write_ticket.lease_ttl_seconds", label: "写入票据租约时间（Write ticket lease TTL）", description: "写入票据 worker 领取后的处理租约秒数。", category: "worker", type: "number", unit: "seconds", default_value: 120, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-write-ticket-worker.service", safety: "guarded", min: 10, max: 3600, step: 10, env_key: "MEMORY_V2_WRITE_TICKET_LEASE_TTL_SECONDS" }, env),
    registryItem({ key: "health.projector_stale_after_ms", label: "投影 worker 过期阈值（Projector stale threshold）", description: "超过该时间未 heartbeat（心跳）时，health gate（健康门禁）标记投影 worker 过期，新审批请求热生效。", category: "health_gate", type: "number", unit: "ms", default_value: 180_000, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 10_000, max: 3_600_000, step: 1000, env_key: "MEMORY_V2_AUTO_APPROVAL_PROJECTOR_STALE_AFTER_MS" }, env),
    registryItem({ key: "health.outbox_blocker_threshold", label: "投影队列阻断阈值（Outbox blocker threshold）", description: "outbox（投影事件队列）积压超过该值时阻断自动审批，新审批请求热生效。", category: "health_gate", type: "number", unit: "events", default_value: 100, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1, max: 100_000, step: 1, env_key: "MEMORY_V2_AUTO_APPROVAL_OUTBOX_BLOCKER_THRESHOLD" }, env),
    registryItem({ key: "health.cache_invalidation_blocker_threshold", label: "缓存失效阻断阈值（Cache invalidation blocker threshold）", description: "缓存失效 backlog（积压）超过该值时阻断自动审批，新审批请求热生效。", category: "health_gate", type: "number", unit: "requests", default_value: 100, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 1, max: 100_000, step: 1, env_key: "MEMORY_V2_AUTO_APPROVAL_CACHE_INVALIDATION_BLOCKER_THRESHOLD" }, env),
    registryItem({ key: "health.qdrant_pg_diff_blocker_threshold", label: "向量库/数据库数量差异阻断阈值（Qdrant/PG diff threshold）", description: "Qdrant（向量库）和 PostgreSQL（关系数据库）的投影数量差超过该阈值时阻断自动审批，新审批请求热生效。", category: "health_gate", type: "number", unit: "records", default_value: 0, writable: true, hot_reloadable: true, requires_restart: false, safety: "guarded", min: 0, max: 10_000, step: 1, env_key: "MEMORY_V2_AUTO_APPROVAL_QDRANT_PG_DIFF_BLOCKER_THRESHOLD" }, env),
    registryItem({ key: "database.connection.idle_timeout_ms", label: "数据库空闲连接超时（PostgreSQL idle timeout）", description: "PostgreSQL 连接池空闲连接超时时间。", category: "database", type: "number", unit: "ms", default_value: 30_000, writable: true, hot_reloadable: false, requires_restart: true, service: "memory-xx-wrapper.service", safety: "guarded", min: 1000, max: 600_000, step: 1000, env_key: "MEMORY_V2_DATABASE_IDLE_TIMEOUT_MS" }, env),
    ...
    envOnlyItem("MEMORY_V2_DATABASE_URL", "数据库连接地址（PostgreSQL URL）", "数据库连接地址，只读脱敏展示。", "postgres://postgres:postgres@127.0.0.1:5432/memory_xx", "memory-xx-wrapper.service", env),
    envOnlyItem("MEMORY_V2_DATABASE_SCHEMA", "数据库 schema（PostgreSQL schema）", "数据库 schema（命名空间）名称。", "memory_xx", "memory-xx-wrapper.service", env),
    envOnlyItem("MEMORY_V2_QDRANT_COLLECTION", "向量集合（Qdrant collection）", "Qdrant（向量库）集合名称。", "memory-xx", "memory-xx-qdrant-projector-worker.service", env),
    envOnlyItem("MEMORY_V2_QUERY_EMBEDDING_CACHE_VERSION", "查询向量缓存版本（Query embedding cache version）", "查询向量缓存版本，切换 embedding generation（嵌入代际）后应更新。", "", "memory-xx-wrapper.service", env),
    envOnlyItem("MEMORY_V2_ADMIN_TOKEN", "管理令牌（Admin token）", "管理 API token（接口令牌），只读脱敏展示。", "", "memory-xx-wrapper.service", env),
  ];
}
